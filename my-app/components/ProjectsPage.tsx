...
export default function ProjectsPage() {
  ...
  const updateParam = useCallback((page: number, year: number) => {
    router.push(`/projetos?page=${page}&year=${year}`);
  }, [router]);
  ...
  const fetchData = useCallback(async (year: number, page: number) => {
    ...
    try {
      ...
      const api = await projectService.listProject(page, year);
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-col md:pl-30 gap-4 p-5 md:p-0">
                  ...
                  <Button
                    link={`projetos/${item.titulo.toLowerCase().replaceAll(" ", "-")}-${item.documentId}`}
                  />
